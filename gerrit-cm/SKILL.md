---
name: gerrit-cm
description: Commit changes based on spec file changelog format [type#id]comment. Use this when committing changes following Gerrit workflow with spec file changelog entries.
license: MIT
trigger:
  - "按照.*changelog.*提交"
  - "commit.*changelog"
...
# Gerrit Commit by Changelog

This skill commits changes following the spec file changelog format for Gerrit workflow.
...
1. Find the spec file in the current directory
2. Extract the first changelog entry
3. Parse type, ID, and comment from the entry
4. Create commit with the format `[type#id]comment`
